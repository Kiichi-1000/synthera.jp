...
1. **ダッシュボード（必須）** — Cloudflare ダッシュボード → **Workers & Pages** → 対象プロジェクト → **Settings** → **Functions / Compatibility flags** で、**Production と Preview の両方**に **`nodejs_compat`** を追加してください。
2. **Compatibility date（必須）** — 同じ画面で `2024-12-30` 以降の日付を設定してください。
3. **リポジトリ（補助）** — ルートに `wrangler.toml` を置き、`compatibility_flags = ["nodejs_compat"]` を保持してください（Git 連携デプロイではダッシュボード設定が優先される場合があります）。
